...
F.module('lib/template', function () {
    ...
    var _TplEngine = function (str, data){
        ...
        _getTpl = function (str){
            ...
            //如果元素存在
            if(ele){
                // 如果是 input 或者 textarea 表单元素，则获取该元素得到value值，否则获取元素的内容
        ...
        _compileTpl = function (str){
            //编译函数体
/*声明template_array 模板容器组*/            var fnBody = "var template_array = [];\n
/*闭包，模板容器组添加成员*/                var fn = (function (data) {\n
/*渲染数据变量的执行函数体*/                var template_key = '';\n
/*遍历渲染数据*/                for(key in data){\n
/*为渲染数据变量的执行函数体添加赋值语句*/                    template_key +=('var' + key + '=data[\"' + key + '\"];')\n
                }\n
/*执行渲染数据变量函数*/                eval(template_key);\n
/*为模板容器组添加成员*/                template_array.push('"+_dealTpl(str)+"');\n
/*释放渲染数据变量函数*/                template_key = null;\n
/*为闭包传入数据*/            })(templateData);\n
/*释放闭包*/            fn = null;\n
/*返回渲染后的模板容器组，并拼接成字符串*/            return template_array.join('');";
            //编译函数
          ...
          return function (str) {
              ...
              eachArray(part, function (partIndex, partValue, partLen) {
                  ...
                  eachArray(item, function (itemIndex, itemValue, itemLen) {
                     ...
                     // 否则，如果 partIndex (层级索引) 对应的层级不是最后一层 作为父层级处理
                     // 否则，该元素无兄弟元素，无子元素
...
~(function () {
    ...
    // 获取页面字面大小，作为创建页面 UI 尺寸参照物
    var FONTSIZE = function () {
        // 获取页面 body 元素字体大小并转化为整数
        return parseInt (document.body.currentStyle ? document.body.currentStyle['fontSize'] : getComputerStyle(document.body, false)['fontSize']);
    }();
    // 视图模型对象
    var VM = function (){
        //组件创建策略对象
